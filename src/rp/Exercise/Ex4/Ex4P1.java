...
public class Ex4P1 {
	...
	public static IGridMap createRectangularGridMap(int xJunctions, int yJunctions, float pointSeparation) {

		int xInset = (int) (pointSeparation / 2);
		int yInset = (int) (pointSeparation / 2);

		float width = xJunctions * pointSeparation;
		float height = yJunctions * pointSeparation;
		...
		return createGridMap(new RPLineMap(lineArray, new Rectangle(0, 0, width, height)), xJunctions, yJunctions, xInset, yInset, pointSeparation);
	}
	...
	private boolean isManuallyApproved(Point from, Point to, HashMap<Point, Point> invalid) {
		Point to1 = invalid.get(from);
		if (to1 != null && to1.equals(to))
			return false;

		Point from1 = invalid.get(to);
		if (from1 != null && from1.equals(from))
			return false;
